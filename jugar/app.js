(() => {
  ...
  function exportGame(state2) {
    ...
    downloadBlob(blob, `loterias-alora-${ymd2}.json`);
  }
  async function exportDayPackage(state2, { downloadDayClosePdf: downloadDayClosePdf2 } = {}) {
    ...
    state2.ui.toast = summary ? `Paquete del d\xEDa ${ymd2}: partida + PDF de cierre` : `Paquete del d\xEDa ${ymd2}: partida (sin cierre reciente)`;
  ...
  function advancePrizeCase(state2, caseId) {
    ...
    if (c.status === "open") {
      ...
      state2.ui.toast = "Caso documentado. Siguiente: presentar a LAE/ONCE.";
      ...
      state2.ui.toast = `Presentado a ${c.org || "organismo"}.`;
...
%%EOF`;
  ...
  function highPrizeAlertHTML() {
    ...
    <div>${escapeHtml(a.clientName || "Cliente")} \xB7 ${escapeHtml(a.productName || "")} \xB7 ${formatEuro(a.amountCents)}</div>
    ...
    <div class="menu-screen">
      ...
      return `<div class="slot">
        ...
        <p class="disclaimer">
          ...
          Juego responsable \xB7 +18. Versi\xF3n ${GAME_VERSION}: encargos, vitrina, hora del sorteo, 5+C, ONCE extras, botes que caen y UI renovada.
        </p>
      </div>
    </div>
  `;
    document.getElementById("btn-new").onclick = () => {
      ...
      showToast("Bienvenida, Miriam. Versi\xF3n 0.9 lista. \xA1A por el mostrador!");
      needsFullRender = true;
      render();
    };
  ...
  function drawNoticeBannerHTML() {
    ...
    if (live.length) {
      ...
      <div class="notice-banner-detail">${escapeHtml(label)} \xB7 m\xE1s comprobaciones en cola</div>
    </div>`;
    }
    ...
    const extraHtml = extras.length ? `<div class="notice-banner sticky-notice">
        ...
        <div class="notice-banner-detail">${escapeHtml(extras.map((e) => e.name).join(" \xB7 "))} \xB7 a la venta hoy</div>
      </div>` : "";
  ...
  function wishlistHTML(client) {
    ...
    return `<ul class="wish-list">${list.map((w) => {
      ...
      return `<li>\u2022 <strong>${escapeHtml(w.productName)}</strong> \xD7 ${w.qty}${extra.length ? ` <span class="muted">(${escapeHtml(extra.join(" \xB7 "))})</span>` : ""}</li>`;
    }).join("")}</ul>`;
  }
  ...
  function renderClientPanel(client) {
    ...
    if (intent === "buy" || intent === "reserve_special" || intent === "abono" || intent === "pena_day" || intent === "pickup" || intent === "showcase_ask") {
      ...
      <div class="client-card intent-${escapeHtml(intent)}">
        <div class="muted">${kindLabel}${trait} \xB7 ${escapeHtml(client.street || "")}${client.specialDay === "birthday" ? " \xB7 Cumplea\xF1os" : client.specialDay === "santo" ? " \xB7 Santo" : ""}${intent === "pickup" ? " \xB7 Encargo listo" : ""}${intent === "showcase_ask" ? " \xB7 Vitrina" : ""}</div>
        <h3>${escapeHtml(client.name)}</h3>
        ${quote ? `<p class="muted">\u201C${escapeHtml(quote)}\u201D</p>` : ""}
        <p>${title}</p>
        ${wishlistHTML(client)}
        ${client.subscription ? `<p class="muted">Suscripci\xF3n: ${escapeHtml(client.subscription)}</p>` : ""}
        ${totalWish ? `<p class="muted">Estimado: ${formatEuro(totalWish)}</p>` : ""}
        <p class="muted">Pago preferido: ${payLabel(client.prefersPayment)}</p>
        <div class="actions">
          <button class="btn primary" id="btn-open-tpv">${intent === "abono" ? "Abrir TPV y confirmar abono" : intent === "pena_day" ? "Abrir TPV (cargar pe\xF1a)" : intent === "pickup" ? "Abrir TPV (entregar)" : intent === "showcase_ask" ? "Abrir TPV (vitrina)" : "Abrir TPV"}</button>
          <button class="btn" id="btn-load-wish">${intent === "abono" ? "Cargar abono" : intent === "pena_day" ? "Cargar pedido pe\xF1a" : intent === "pickup" ? "Cargar encargo" : intent === "showcase_ask" ? "Cargar n\xBA vitrina" : "Cargar petici\xF3n"}</button>
  ...
  function bindClientActions() {
    ...
    if (openTpvBtn) {
      openTpvBtn.onclick = () => {
        ...
        if (["pena_day", "pickup", "showcase_ask", "abono"].includes(client.intent) && (((_a = client.wishlist) == null ? void 0 : _a.length) || client.request)) {
          loadWishlistIntoTpv(client);
          if ((_b = state.ui.tpv) == null ? void 0 : _b.message) showToast(state.ui.tpv.message);
        } else if ((_c = client.wishlist) == null ? void 0 : _c.length) {
          loadWishlistIntoTpv(client);
  ...
  function renderTpv() {
    ...
    <div class="shell">
      ...
      (d) => `<button class="btn" style="width:100%;margin-bottom:4px;justify-content:flex-start" data-sell-showcase="${d.id}">n\xBA ${escapeHtml(d.number)} \xB7 ${escapeHtml(d.productName)} \xD7${d.qty}</button>`
    ).join("")}
                  </div>` : ""}
          </aside>
        </div>
    ...
    <div class="product-sheet-overlay" id="product-sheet">
      <div class="product-sheet panel">
        <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start">
          <div>
            <div class="muted">${escapeHtml(p.org || "")} \xB7 ${escapeHtml(p.tpvCategory || "")}</div>
        ...
        <div class="close-summary" style="margin-top:12px">
          ...
          <div class="stat-row"><span>Mec\xE1nica</span><strong>${escapeHtml(p.trait || p.numberMode || "\u2014")}</strong></div>
          <div class="stat-row"><span>Premio orientativo</span><strong>${escapeHtml(p.topPrizeHint || "\u2014")}</strong></div>
          <div class="stat-row"><span>D\xEDas de sorteo</span><strong>${escapeHtml(days)}</strong></div>
    ...
    <div class="shell">
      ...
      return `<div class="stat-row"><span>${org} ventas</span><strong>${formatEuro(o.sales)}</strong></div>
          ...
          ${settle ? `<div class="close-summary">
                  ${settlementExplain(settle).map((line) => `<p class="muted" style="margin:4px 0">${escapeHtml(line)}</p>`).join("")}
  ...
  function renderStock() {
    ...
    <div class="shell">
      ...
      return `<div class="stock-item">
                  <span><strong>${escapeHtml(p.name)}</strong> <span class="muted">(${p.org} \xB7 coste ~${formatEuro(cost)})</span></span>
                  <span>${qty}
      ...
      (o) => `<div class="log-item">${o.status}${o.supplier ? " \xB7 PROVEEDOR" : ""}${o.special ? " \xB7 ENCARGO" : ""} \xB7 ${escapeHtml(o.productName)} \xD7${o.qty} \xB7 ${o.arriveOnYmd}${o.totalCostCents != null ? ` \xB7 ${formatEuro(o.totalCostCents)}` : ""}${o.clientName ? ` \xB7 ${escapeHtml(o.clientName)}` : ""}</div>`
  ...
  function renderPrize() {
    ...
    <div class="shell">
      ...
      (t) => `<div class="log-item">
                        <strong>${escapeHtml(t.id)}</strong> \xB7 ${escapeHtml(t.clientName || "\u2014")} \xB7 ${escapeHtml(t.productName)}
                        <br/>${escapeHtml(formatSelection(t))} \xB7 <strong>${formatEuro(t.prizeCents)}</strong>
                        <span class="muted"> \xB7 ${escapeHtml(t.status)}</span>
    ...
    <div class="shell">
      ...
      <div class="layout" style="grid-template-columns:280px 1fr">
        ...
        <section class="panel">
          ...
          <p class="muted">No salen de tu caja. T\xFA avanzas los pasos: abierto \u2192 documentado \u2192 presentado \u2192 liquidado (paga LAE/ONCE).</p>
          <div class="log">
            ${list.length ? list.map((c) => {
      const st = PRIZE_MGMT_STATUS[c.status] || c.status;
      const btn = c.status !== "settled" ? `<button class="btn primary" style="margin-top:8px;padding:6px 12px" data-adv-case="${c.id}">${nextLabel[c.status] || "Avanzar"}</button>` : "";
      return `<div class="log-item">
                        <strong>${escapeHtml(c.clientName)}</strong> \xB7 ${formatEuro(c.amountCents)} \xB7 ${escapeHtml(c.productName)}
                        <br/>Ticket ${escapeHtml(c.ticketId || "\u2014")} \xB7 ${escapeHtml(c.org || "")} \xB7 ${c.level}
  ...
  function renderDayResults() {
    ...
    <div class="shell">
      ...
      <div class="panel day-results" style="margin-top:16px;max-width:720px">
        ...
        <p class="muted">Cierre de ${escapeHtml(s.date)} \xB7 siguiente laborable ${escapeHtml(s.nextDay)}</p>
        <div class="close-summary">
        ...
        ${settle ? `<h3 style="margin-top:16px">Liquidaci\xF3n</h3>
               ...
               <div class="close-summary">
                 ${settlementExplain(settle).slice(2).map((line) => `<div class="stat-row"><span>${escapeHtml(line)}</span></div>`).join("")}
               </div>` : ""}
        ${((_a = s.nextDayReasonSkip) == null ? void 0 : _a.length) ? `<p class="muted" style="margin-top:12px">D\xEDas saltados: ${escapeHtml(s.nextDayReasonSkip.join(", "))}</p>` : ""}
        <div class="actions" style="margin-top:18px">
  ...
  function renderShowcase() {
    ...
    <div class="shell">
      ...
      (d) => `<div class="showcase-item">
                        <strong>n\xBA ${escapeHtml(d.number)}</strong>
                        <div>${escapeHtml(d.productName)} \xD7${d.qty}</div>
                        <div class="muted">${formatEuro(d.unitCents * d.qty)}${d.note ? ` \xB7 ${escapeHtml(d.note)}` : ""}</div>
      ...
      (p) => `<option value="${p.id}">${escapeHtml(p.short || p.name)} \xB7 ${escapeHtml(p.org || "")}</option>`
    ).join("")}
              </select>
            </label>
