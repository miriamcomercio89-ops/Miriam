...
def approx_ctx(city, seed_key):
    rc = B.rng(B.h32(*seed_key, "ctx"))
    edge = rc() < 0.22
    central = (not edge) and rc() < 0.4
    dist = "Poligono" if edge else ("Centro" if central else "")
    beach = rc() < 0.05
    metro = city["pop"] >= 250000 and rc() < 0.35
    return {
...
def build_brand_events(cities):
    ...
    (pick_brand_sequence). Devuelve una lista de 50 listas de (city, slot)."""
    ...
    return events_by_brand


def build_brand_records(bid_idx: int, events: list, econ: dict):
    ...
    formato e inversión estimada (alquiler/compra + obra + mobiliario + stock inicial)."""
    ...
    for slot_i, (city, slot) in enumerate(ev):
        ...
        records.append({
            ...
            "admin1": city["admin1"], "pop": city["pop"],
            "size_id": size_id, "size_name": sl, "seats": seats, "m2": m2,
            "owned": owned, "alq_compra": alq_compra, "construction": construction,
            "furniture": furniture, "stock": stock, "total_inv": total_inv,
            "first_for_cc": first_for_cc,
        })
...
def render_phase_pdf(path: Path, bid, name, cuisine, tier, tag, phase_no, slice_records,
    ...
    doc.h2(f"Locales de la fase {phase_no}")
    rows = []
    for r in slice_records:
        rows.append([
            f"{r['city']} ({r['country']})",
            r["size_name"],
            str(r["seats"]),
            ("C " if r["owned"] else "A ") + fmt_compact(r["alq_compra"]),
            fmt_compact(r["construction"]),
            fmt_compact(r["furniture"]),
            fmt_compact(r["stock"]),
            fmt_compact(r["total_inv"]),
        ])
    doc.table(
        ["Ciudad (país)", "Formato", "Af.", "Alq./Compra", "Obra", "Mobil.", "Stock", "Total"],
        rows,
        [143, 82, 24, 68, 58, 58, 58, CONTENT_W - 143 - 82 - 24 - 68 - 58 - 58 - 58],
        size=FONT_SIZE,
        align=["l", "l", "r", "r", "r", "r", "r", "r"],
        phase_label=f"Fase {phase_no} de {N_PHASES} · {period_label}",
        title=f"{name} — fase {phase_no}",
        total_row=["TOTAL fase", "", "", "", "", "", "", fmt_compact(invest_this)],
    )
    doc.para(
        "\"A\" = inversión inicial en alquiler (fianza y primeros meses); \"C\" = compra del local. "
...
def process_brand(bid_idx: int, n_phases: int, tmp_dir: Path):
    ...
    records, bid, name, cuisine, tier, tag = build_brand_records(bid_idx, events_by_brand[bid_idx], econ)
    total = len(records)
