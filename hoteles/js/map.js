/* Horizon Hotels — mapa Leaflet, agregación por país/ciudad y clusters de marca */
(function (global) {
  ...
  function brandIcon(brand, extra, size = 36) {
    const badge = extra ? `<span class="pin-badge">${extra > 999 ? "999+" : extra}</span>` : "";
    return L.divIcon({
      className: "brand-pin",
  ...
  function filtered() {
    ...
    return game.state.hotels.filter((r) => {
  ...
  function refresh() {
    try {
      ...
      if (!refreshErrorShown && game && typeof UI !== "undefined" && UI.toast) {
        ...
        UI.toast("Algún hotel tenía datos incompletos: se ha omitido para no romper el mapa. El resto se ve con normalidad.", true);
      }
    }
  }
  ...
  function addHotel(r) {
    ...
    const m = L.marker([r.lat, r.lon], { icon: brandIcon(brand, 0, 82), keyboard: false });
    if (heat) {
  ...
  function jumpProfit(best) {
    const list = game.state.hotels.filter((r) => r.status === "abierto");
  ...
  function dotsThumb() {
    ...
    g.fillStyle = "#e85d04";
    const list = (game.state && game.state.hotels) || [];
