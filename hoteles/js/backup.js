...
(function (global) {
  const DB_NAME = "horizon-hotels-backup-db";
  const DB_VER = 1;
  const STORE_NAME = "handles";
  const HANDLE_KEY = "backup-file";
  const FALLBACK_KEY = "horizon-hotels-backup-fallback";
  ...
  async function enable(suggestedName) {
    ...
    try {
      const h = await global.showSaveFilePicker({
        suggestedName: suggestedName || "horizon-hotels-backup.json",
        types: [{ description: "Copia de seguridad de Horizon Hotels", accept: { "application/json": [".json"] } }],
      });
      handle = h;
      fileName = h.name || "";
      active = true;
  ...
  function downloadNow(state) {
    try {
      ...
      a.href = url;
      a.download = "horizon-hotels-autoguardado.json";
