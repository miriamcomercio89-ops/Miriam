...
(function () {
  ...
  const game = {
    ...
    uiTab: "locales",
    openId: null,
    filters: { brand: "", segment: "", country: "", status: "", profit: "", sort: "new" },
    _lastUi: 0,
    _lastSave: 0,
    _lastNews: 0,
    _busyGeo: false,
  ...
  function newGame() {
    try {
      ...
      const name = ((input && input.value) || "Horizon Hotels 2000").trim() || "Horizon Hotels 2000";
      ...
      UI.toast("Horizon Hotels abre su libro de reservas. Caja: 5.000.000 €. 1 de enero de 2000.");
    } catch (err) {
      console.error(err);
  ...
  function continueGame() {
    try {
      ...
      const idx = (function () {
        try {
          return JSON.parse(localStorage.getItem("horizon-hotels-index") || "[]");
        } catch {
          return [];
        }
      })();
  ...
  function advance(gameMs, force) {
    ...
    s.gameTime = to;
    SIM.tickReopen(s, to);
    const n = s.hotels.length;
    if (n > 2500 && gameMs > 6 * 3600000) {
      const step = Math.ceil(n / 400);
      for (let i = 0; i < n; i += step) SIM.settleOne(s, s.hotels[i], s.hotels[i].lastSim || from, to);
      for (const r of s.hotels) if (r.lastSim < to - 3600000) r.lastSim = to;
    } else {
      SIM.settleAll(s, to);
    }
  ...
  function confirmBuild(brandId, htypeId, place, quote, extra) {
    const s = game.state;
  ...
  try {
    ...
    console.error(err);
    alert("Error al iniciar Horizon Hotels: " + (err && err.message ? err.message : err));
  }
  try {
