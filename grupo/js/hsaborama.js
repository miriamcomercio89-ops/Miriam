...
(function (global) {
  ...
  function applyBook(state, r, brand, pl) {
    ...
    const fair = HSIM.fairADR(brand, pl);
  ...
  function runManager(state, r, brand, gameMs) {
    ...
    if (r.cleanliness < 42 && skill >= 45) {
      const c = 260 * pl * (HSIM.sizeOf(r.htype).rooms / 40);
    ...
    const seg = HBRAND.SEGMENTS[brand.segment];
    const wantAmenities = [];
  ...
  function alerts(state) {
    ...
    const debt = HSIM.debtTotal ? HSIM.debtTotal(state) : 0;
  ...
  function estimateHover(lat, lon, gameMs) {
    ...
    const q = HSIM.buildQuote(HBRAND.list[0], "urbano_pequeno", dummy, gameMs || Date.UTC(2000, 0, 1));
    return {
