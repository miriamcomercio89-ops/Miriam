/* Horizon Group — partidas: localStorage primero (funciona abriendo el HTML), IndexedDB
   opcional. Una sola partida combina restaurantes (Horizon Restaurant Group) y hoteles
   (Horizon Hotels) en los arrays `restaurants` y `hotels`, con una caja común. Claves
   propias (horizon-group-*) para no colisionar con los simuladores independientes de
   restaurantes u hoteles si conviven en el mismo origen/navegador. */
(function (global) {
  const DB_NAME = "horizon-group-db";
  const DB_VER = 1;
  const META = "horizon-group-meta";
  const INDEX = "horizon-group-index";
  ...
  function lsSaveKey(id) {
    return "horizon-group-save-" + id;
  }

  const START_CASH = 30000000;
      ...
      news: [
        {
          ...
          kind: "editorial",
          decade: 2000,
          text: "1 de enero de 2000. Horizon Group abre libros: Horizon Restaurant Group y Horizon Hotels arrancan juntas, con una sola caja. El mundo es un mapa en blanco.",
        },
      ],
      competitors,
      revByCountry: {},
      cashHistory: [{ t: Date.UTC(2000, 0, 1, 8, 0, 0), v: START_CASH }],
      books: {},
      yearbooks: [],
    };
  }
  ...
  function summaryOf(state) {
    const nR = (state.restaurants || []).length;
    const nH = (state.hotels || []).length;
    const countries = new Set([...(state.restaurants || []).map((r) => r.country), ...(state.hotels || []).map((r) => r.country)]);
    return {
      id: state.id,
      name: state.name,
  ...
  async function load(id) {
    ...
    if (s) {
      ...
      if (!Array.isArray(s.restaurants)) s.restaurants = [];
      if (!Array.isArray(s.hotels)) s.hotels = [];
    }
    return s;
  }
  ...
  function exportJSON(state) {
    ...
    U.download(`horizon-group-${state.name.replace(/\s+/g, "_")}-${new Date(state.gameTime).toISOString().slice(0, 10)}.json`, blob);
  }
  ...
  function parseImport(text) {
    ...
    if (!data || !data.gameTime || (!Array.isArray(data.restaurants) && !Array.isArray(data.hotels))) throw new Error("Archivo no válido");
    ...
    /* Partidas antiguas (del simulador de restaurantes u hoteles por separado, o
       exportadas antes de algún campo nuevo) no deben romper el mapa ni el resto
       del juego al importarlas a Horizon Group. */
    stripTransient(data);
  ...
  global.STORE = { blankState, summaryOf, listSlots, save, load, remove, exportJSON, parseImport, meta, setMeta, START_CASH };
})(window);
