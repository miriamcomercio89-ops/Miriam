...
(function (global) {
  const DB_NAME = "horizon-group-backup-db";
  const DB_VER = 1;
  const STORE_NAME = "handles";
  const HANDLE_KEY = "backup-file";
  const FALLBACK_KEY = "horizon-group-backup-fallback";
  ...
  async function enable(suggestedName) {
    ...
    try {
      const h = await global.showSaveFilePicker({
        suggestedName: suggestedName || "horizon-group-backup.json",
  ...
  function downloadNow(state) {
    try {
      ...
      a.href = url;
      a.download = "horizon-group-autoguardado.json";
