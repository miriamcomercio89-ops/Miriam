...
export async function downloadHotelPdf(args: {
  ...
  if (photoSrc) photoPng = await anyImageToPng(photoSrc, 1400)
  if (photoPng) {
    const photoH = 52
    doc.setFillColor(240, 242, 244)
    doc.roundedRect(margin, y, pageW - margin * 2, photoH, 3, 3, 'F')
    await embedImage(doc, photoPng, margin, y, pageW - margin * 2, photoH)
    y += photoH + 8
  }
