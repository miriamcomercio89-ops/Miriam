    ...
    case "urban": {
      ...
      return `
        <g transform="translate(${shift * 0.3} 0)">
          ...
          ${[0, 1, 2, 3, 4].map(
        (row) => [0, 1].map(
          (col) => `<rect x="${22 + col * 8}" y="${128 - 28 - h1 + 10 + row * 10}" width="5" height="5" rx="0.8" fill="#F7F3EA" opacity="${0.2 + v * 0.25}"/>`
        ).join("")
      ).join("")}
          ${[0, 1, 2, 3, 4, 5].map(
        (row) => [0, 1, 2].map(
          (col) => `<rect x="${48 + col * 8}" y="${128 - 28 - h2 + 8 + row * 9}" width="5" height="5" rx="0.8" fill="#F7F3EA" opacity="${0.15 + v2 * 0.2}"/>`
        ).join("")
      ).join("")}
...
function buildSubsidiaryLogoSvg(sub, size = 128) {
...
function logoComplexityHint(style) {
  switch (style) {
    ...
    case "nature":
      return "dosel / monta\xF1a vegetal";
    case "luxury":
...
function pickLogoScale(size) {
  return clamp(size, 48, 1024);
}
export {
  buildSubsidiaryLogoSvg,
  logoComplexityHint,
  pickLogoScale,
  subsidiaryLogoDataUrl
};
