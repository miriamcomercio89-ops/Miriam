# -*- coding: utf-8 -*-
"""
PDF completo — Línea 5 de tranvía (documento Straßenbahnnetz: «11»).
Orden EMT: 5 · Código original: 11.
"""
...
OUT = Path(__file__).resolve().parent / "lineas" / "linea_05_tranvia.pdf"
...
LINE = {
    ...
    "code_doc": "11",
    "mode": "Tranvía",
    "name": "Barrio del Farallón – Calle del Peñón",
    "name_old": "Kaiserebersdorf, Zinnergasse → Raxstraße",
    "color": "#8B4513",
    "text": "#FFFFFF",
    "kind": "Radial sur / depósito – estación – Peñón",
    "service": "05:35 – 23:40",
    "distance_km": 6.8,
    "runtime_min": 24,
    "headway_peak": 8,
    ...
    "fleet_units": 9,
    "vehicles": [
        ("ULF A1 (Viena)", "Base", 140, "principal"),
        ("Tatra KT4D", "DLC Tatra KT4D", 150, "refuerzo / clásico"),
    ],
    "depot": "Depot Süd / Cocheras Favorita",
    "zones": ["A", "B", "C"],
    "notes": (
        "En el documento es la «11»; en EMT es la Línea 5. Une el Farallón (sureste) con "
        "el Peñón pasando por Cocheras Favorita, Laurel, Cantera, Juzgados, Coso y la "
        "Estación María del Mar. Ideal para sacar material del depósito al sur profundo."
    ),
}

STOPS = [
    ("Kaiserebersdorf, Zinnergasse", "Barrio del Farallón", "Eglundsdorf", False, "C", True),
    ...
    ("Hauptbahnhof", "Estación María del Mar", "Hauptbahnhof", True, "A", True),
    ("Laxenburger Straße, Gudrunstraße", "Avenida de la Caleta", "Goethestraße", False, "B", False),
    ("Raxstraße", "Calle del Peñón", "Johann-Friedrich-Gasse", False, "C", True),
]
...
def build():
    ...
    brows = [[Paragraph("<b>Franja</b>", STY["Tiny"]),
              ...
              Paragraph("<b>Ej. salidas cabecera Farallón</b>", STY["Tiny"])]]
    ...
    story.append(Paragraph(
        "Laborable ×1,0 · Verano ×1,15 · Feria ×0,80 · Navidad ×0,90 · "
        "Día de partido: enlace en Estación hacia R-EST (T6/5/9 del pack general).",
        STY["Body"]))
    ...
    hubs_txt = [
        ("Barrio del Farallón", "Cabecera sureste; buses locales A4 / nocturnos."),
        ("Cocheras Favorita", "Entrada/salida de depósito; trasbordo a L3."),
        ("Estación María del Mar", "Hub S/U; cruce con L4 y buses del Perchel."),
        ("Calle del Peñón", "Cabecera sur; barrio residencial / fin de línea."),
    ]
    for h, t in hubs_txt:
    ...
    checks = [
        ...
        "Renombrar las 9 paradas (varias ya en L3/L4)",
        "Cabeceras: Barrio del Farallón ↔ Calle del Peñón",
        "Asignar ULF A1 + KT4D (opcional)",
        ...
        "Comprobar vía Farallón → Depot → Estación → Peñón en capa Tram",
        "Probar Free Roam (~24 min)",
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(
        "Siguiente del documento: suele ser la 18 → Línea 6 EMT.",
        STY["Small"]))
