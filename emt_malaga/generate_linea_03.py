# -*- coding: utf-8 -*-
"""
PDF completo — Línea 3 de tranvía (en el documento Straßenbahnnetz aparece como «6»).
Orden EMT: 1, 2, 3… · Código original del plano: 6.
"""
...
OUT = Path(__file__).resolve().parent / "lineas" / "linea_03_tranvia.pdf"
OUT.parent.mkdir(parents=True, exist_ok=True)

# --- Identidad línea ---
# EMT Málaga: código 3 (3ª línea que montamos).
# Documento Carl B. / captura: etiquetada como «6».
LINE = {
    "code": "3",
    "code_doc": "6",
    "mode": "Tranvía",
    "name": "Iglesia de la Memoria – Plaza de la Concordia",
    "name_old": "Burggasse, Gedächtniskirche → Reumannplatz",
    "color": "#F5A623",
    "text": "#1A1A1A",
    "kind": "Corredor noroeste – sur",
    "service": "05:40 – 23:45",
    "distance_km": 7.4,
    "runtime_min": 28,
    "headway_peak": 7,
    ...
    "vehicles": [
        ...
        ("ULF A1 (Viena)", "Base", 140, "refuerzo / calles más exigentes"),
    ],
    "depot": "Depot Süd / Depósito Sur",
    "zones": ["A", "B"],
    "notes": (
        "En el documento original es la línea «6»; en la red EMT Málaga la numeramos como "
        "Línea 3 (tercera ficha). Une el noroeste (Memoria / deportes / hospital) con el sur "
        "(Concordia) vía Arco del Este, Cinturón, Estación Norte y Juzgados. "
        "Varias paradas con U; Hauptbahnhof Nord también con S en el plano."
    ),
}

# (plano actual, ES, CTS mapa, u_bahn, zona, hub)  — u_bahn True también si trae S
STOPS = [
    ("Burggasse, Gedächtniskirche", "Iglesia de la Memoria", "Friedrich-Schneider-Straße", False, "B", True),
    ("Hütteldorfer Straße, Stadthalle", "Palacio de Deportes", "Helene-Weber-Straße", False, "B", False),
    ("Felberstraße-AKH", "Hospital Norte", "Krankenhaus Nord", True, "B", False),
    ("Christian-Broda-Platz", "Plaza del Mercado Norte", "Humboldtplatz", False, "A", True),
    ("Stubentor", "Arco del Este", "Oberer Ring", True, "A", True),
    ("Arbeitergasse, Gürtel", "Cinturón Obrero", "Ringensteinstraße", False, "B", False),
    ("Hauptbahnhof Nord", "Estación Norte", "Hauptbahnhof West", True, "A", True),  # U+S
    ("Enkplatz", "Plaza del Coso", "Konstanzgasse", True, "B", False),
    ("Landesgerichtsstraße, Rathausplatz", "Juzgados – Ayuntamiento", "Unterer Ring", True, "A", False),
    ("Braunhubergasse", "Calle de la Cantera", "Davidsweg", False, "B", False),
    ("Gellertplatz", "Plaza del Laurel", "Toriyama Allee", False, "B", False),
    ("Reumannplatz", "Plaza de la Concordia", "Shuji-Nakamura-Platz", True, "B", True),
]
...
def build():
    ...
    brows = [[Paragraph("<b>Franja</b>", STY["Tiny"]),
              ...
              Paragraph("<b>Ej. salidas cabecera Memoria</b>", STY["Tiny"])]]
    ...
    story.append(Paragraph(
        "Laborable ×1,0 · Verano ×1,15 · Feria ×0,75 · Navidad ×0,90 · "
        "Día de partido: refuerzo ligero si hay trasbordo a líneas de Estadio en Concordia / Depot.",
        STY["Body"]))
    ...
    hubs_txt = [
        ("Iglesia de la Memoria", "Cabecera NO; enlace a Palacio de Deportes y Hospital Norte."),
        ("Plaza del Mercado Norte", "Acceso a Universidad / Anillo en otras líneas."),
        ("Arco del Este", "Cruce con L1 y L2; muchas correspondencias del anillo."),
        ("Estación Norte", "Trasbordo S/U en el plano; enlace a L5 / buses de estación."),
        ("Plaza de la Concordia", "Cabecera sur; Depot Sur, Torres del Sur y Farallón."),
    ]
    for h, t in hubs_txt:
    ...
    checks = [
        f"Crear línea código «{LINE['code']}» (o «{LINE['code_doc']}» si prefieres el número del documento) · HEX {LINE['color']}",
        "Renombrar las 12 paradas (reutilizar nombres ya puestos en L1/L2 si existen)",
        "Cabeceras: Iglesia de la Memoria ↔ Plaza de la Concordia",
        "Asignar R2.2b (principal) + ULF A1 (refuerzo)",
        ...
        "Comprobar vía NO → anillo → sur (Depot / Concordia) en capa Tram",
        "Probar Free Roam de punta a punta antes de IA",
        "Timetable Generator: 4 franjas de la sección 3",
    ]
    for c in checks:
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(
        "Siguiente paso: pásame la siguiente del documento (suele ser la 10) y la numeramos como Línea 4.",
        STY["Small"]))
