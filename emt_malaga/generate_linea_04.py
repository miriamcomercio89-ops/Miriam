# -*- coding: utf-8 -*-
"""
PDF completo — Línea 4 de tranvía (en el documento Straßenbahnnetz aparece como «10»).
Orden EMT: 1, 2, 3, 4… · Código original del plano: 10.
"""
...
OUT = Path(__file__).resolve().parent / "lineas" / "linea_04_tranvia.pdf"
...
LINE = {
    ...
    "code_doc": "10",
    "mode": "Tranvía",
    "name": "Colinas del Oeste – Estación María del Mar",
    "name_old": "Dornbach → Hauptbahnhof",
    "color": "#2E8B57",
    "text": "#FFFFFF",
    "kind": "Radial corta / oeste → estación",
    "service": "05:50 – 23:30",
    "distance_km": 5.2,
    "runtime_min": 18,
    "headway_peak": 8,
    ...
    "vehicles": [
        ...
        ("DÜWAG GT6", "DLC GT6", 121, "refuerzo / patrimonial"),
    ],
    "depot": "Depot Süd / Depósito Sur",
    "zones": ["A", "B", "C"],
    "notes": (
        "En el documento original es la línea «10»; en EMT Málaga la numeramos como "
        "Línea 4. Radial corta: baja desde Colinas del Oeste (Dornbach) por Olivar y Junta "
        "del Oeste, gira al este en Teatro Cervantes y termina en la Estación María del Mar. "
        "Solo 6 paradas: ideal para empezar a montar tras L1–L3. Cabecera con S/U en estación."
    ),
}
...
STOPS = [
    ...
    ("Bauernfeldplatz", "Plaza de los Labradores", "Iwatapark", False, "A", False),
    ("Hauptbahnhof", "Estación María del Mar", "Hauptbahnhof", True, "A", True),  # S+U
]
...
def build():
    ...
    story.append(Paragraph(
        "Laborable ×1,0 · Verano ×1,15 · Feria ×0,70 (refuerzo fuerte oeste→estación) · "
        "Navidad ×0,85 · Día de partido: útil como acceso a Estación para enlazar R-EST.",
        STY["Body"]))
    ...
    hubs_txt = [
        ("Colinas del Oeste", "Cabecera oeste compartida con L2; cocheras / Olivar."),
        ("Teatro Cervantes", "Giro al este; correspondencia con L1 y anillo."),
        ("Estación María del Mar", "Cabecera este; S/U en el plano; hub principal de la red."),
    ]
    for h, t in hubs_txt:
    ...
    checks = [
        ...
        "Renombrar las 6 paradas (varias ya existen por L1–L3)",
        "Cabeceras: Colinas del Oeste ↔ Estación María del Mar",
        "Asignar ULF A1 (principal) + GT6 (opcional)",
        ...
        "Comprobar tramo oeste → Teatro → estación en capa Tram",
        "Probar Free Roam (recorrido corto, ~18 min)",
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(
        "Siguiente del documento: suele ser la 11 → la numeraremos como Línea 5.",
        STY["Small"]))
