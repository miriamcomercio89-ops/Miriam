# -*- coding: utf-8 -*-
"""
PDF completo — Línea 6 de tranvía (documento: «18»).
Orden EMT: 6 · Código original: 18.
"""
...
OUT = Path(__file__).resolve().parent / "lineas" / "linea_06_tranvia.pdf"
...
LINE = {
    ...
    "code_doc": "18",
    "mode": "Tranvía",
    "name": "Iglesia de la Memoria – Estadio Costa del Sol",
    "name_old": "Burggasse, Gedächtniskirche → Stadion",
    "color": "#00A3A1",
    "text": "#FFFFFF",
    "kind": "Corredor NO – sur – este / ocio y eventos",
    "service": "05:30 – 00:30",
    "distance_km": 11.2,
    "runtime_min": 38,
    "headway_peak": 8,
    "headway_off": 12,
    "headway_eve": 15,
    "demand": "Alta (+ partidos)",
    "target_load": "70–95 %",
    "fleet_units": 12,
    "vehicles": [
        ("Flexity Wien", "DLC Flexity", 210, "principal / eventos"),
        ("GT8N Mannheim", "DLC GT8N", 180, "refuerzo"),
    ],
    "depot": "Depot Süd / Cocheras Favorita",
    "zones": ["A", "B", "C"],
    "notes": (
        "Documento «18» → EMT Línea 6. Comparte el tramo Memoria→Cantera con la L3 "
        "y continúa por Cocheras Favorita hacia el este hasta el Estadio Costa del Sol. "
        "Línea clave del plan R-EST (refuerzo en partidos/conciertos)."
    ),
}
...
STOPS = [
    ...
    ("Hauptbahnhof Nord", "Estación Norte", "Hauptbahnhof West", True, "A", True),
    ...
    ("Siccardsburggasse, Betriebsdepot Favoriten", "Cocheras Favorita", "Jakob-de-Zoet-Straße", False, "B", True),
    ("Schlachthausgasse", "Antiguo Matadero", "Löwenstraße", True, "B", False),
    ("Ludwig-Koeßler-Platz", "Plaza del Farolillo", "Sonnengasse", False, "B", False),
    ("Dr.-Natterer-Gasse", "Calle de la Higuera", "Lauritostraße", False, "B", False),
    ("Meiereistraße, Handelskai", "Muelle del Comercio", "Eichenweg", False, "B", False),
    ("Stadion", "Estadio Costa del Sol", "Am Weidensee", True, "C", True),
]
...
def build():
    ...
    story.append(Paragraph(
        "Laborable ×1,0 · Verano ×1,10 · Feria ×0,80 · Navidad ×0,90 · "
        "Día de partido: ACTIVAR plan R-EST — intervalo hasta cada 4 min, +4 coches Flexity.",
        STY["Body"]))
    ...
    hubs_txt = [
        ("Iglesia de la Memoria", "Cabecera NO compartida con L3."),
        ("Arco del Este", "Cruce con L1/L2/L3."),
        ("Estación Norte", "Trasbordo S/U; acceso desde L4/L5 por estación."),
        ("Cocheras Favorita", "Depósito; refuerzos de evento."),
        ("Estadio Costa del Sol", "Cabecera este; R-EST y buses 5/9."),
    ]
    ...
    checks = [
        ...
        "Renombrar las 16 paradas (tramo inicial = L3 + extensión este)",
        "Cabeceras: Iglesia de la Memoria ↔ Estadio Costa del Sol",
        "Asignar Flexity Wien + GT8N",
        f"Intervalo punta {LINE['headway_peak']} min (R-EST: 4 min) · valle {LINE['headway_off']} · tarde {LINE['headway_eve']}",
        f"Servicio {LINE['service']} · depósito {LINE['depot']}",
        "Comprobar vía hasta Am Weidensee (Estadio) en capa Tram",
        "Probar Free Roam (~38 min) y ensayo R-EST",
        "Timetable Generator: 4 franjas + plantilla evento",
    ]
    for c in checks:
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(
        "Siguiente del documento: suele ser la 31 → Línea 7 EMT.",
        STY["Small"]))
