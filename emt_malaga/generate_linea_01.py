# -*- coding: utf-8 -*-
"""
PDF completo — Línea 1 de tranvía (Straßenbahnnetz).
...
OUT = Path(__file__).resolve().parent / "lineas" / "linea_01_tranvia.pdf"
...
LINE = {
    "code": "1",
    "mode": "Tranvía",
    "name": "Plaza del Faro – Alameda del Parque",
    "name_old": "Stefan-Fadinger-Platz → Prater Hauptallee",
    "color": "#C8102E",
    "text": "#FFFFFF",
    "kind": "Diametral / eje anillo",
    "service": "05:30 – 00:15",
    "distance_km": 9.8,
    "runtime_min": 34,
    "headway_peak": 6,
    ...
    "fleet_units": 14,
    "vehicles": [
        ("Flexity Wien", "DLC Flexity", 210, "principal"),
        ("ULF B1 (Viena)", "Base", 180, "refuerzo / reserva"),
    ],
    "depot": "Depot Süd / Depósito Sur",
    "zones": ["A", "B"],
    "notes": (
        "Columna vertebral del anillo: une el sur (Faro) con el parque noreste "
        "pasando por Ayuntamiento, Ópera, Anillo Norte, Puerto y Universidad. "
        "Varias paradas con correspondencia tipo metro (icono U en el plano original)."
    ),
}

# Paradas en orden: (nombre_actual_plano, nombre_ES, nombre_CTS_mapa, u_bahn, zona, hub)
STOPS = [
    ("Stefan-Fadinger-Platz", "Plaza del Faro", "Lisa-Meitner-Platz", False, "B", True),
    ("Matzleinsdorfer Platz", "Plaza de los Jardines", "Sternengasse", True, "A", True),
    ...
    ("Schottenring", "Plaza del Anillo Norte", "Puls Center / Olaf-Meier-Platz", True, "A", True),
    ("Salztorhof", "Muelle de la Sal", "Josefshof", False, "A", False),
    ...
    ("Schottentor", "Plaza Universidad", "Universität / Bernhardsdom", True, "A", True),
    ("Radetzkyplatz", "Plaza de la Palmera", "Danielsplatz", False, "A", False),
    ("Prater Hauptallee", "Alameda del Parque", "Lichterallee", False, "B", True),
]
...
def build():
    ...
    brows = [[Paragraph("<b>Franja</b>", STY["Tiny"]),
              ...
              Paragraph("<b>Ej. salidas cabecera Faro</b>", STY["Tiny"])]]
    ...
    story.append(Paragraph(
        "Laborable ×1,0 · Verano ×1,15 (menos frecuente) · Feria ×0,75 (más frecuente) · "
        "Navidad ×0,90 · Día de partido: sin cambio en L1 (refuerza T6/5/9 hacia estadio).",
        STY["Body"]))
    ...
    hubs_txt = [
        ("Plaza del Ayuntamiento", "Cruce con líneas de anillo y buses al Perchel / estación."),
        ("Plaza de la Ópera", "Núcleo turístico; enlace Circular Centro y patrimonial TP."),
        ("Plaza del Anillo Norte", "Salidas al norte (Nuevo Norte, Hospital, Polígono)."),
        ("Plaza del Puerto", "Este / muelle; enlaces a Estadio en otras líneas."),
        ("Plaza Universidad", "Campus; buses U1/U2 en días lectivos."),
        ("Alameda del Parque", "Cabecera este; parque y cementerio en otras líneas."),
        ("Plaza del Faro", "Cabecera sur; enlace a Concordia / Depot Sur."),
    ]
    for h, t in hubs_txt:
    ...
    checks = [
        f"Crear línea código «{LINE['code']}» (o T1) color HEX {LINE['color']}",
        "Renombrar las 18 paradas con la columna EMT Málaga",
        f"Cabeceras: Plaza del Faro ↔ Alameda del Parque",
        "Asignar Flexity Wien (principal) + ULF B1 (reserva)",
        ...
        "Marcar hubs ★ y comprobar vías de tranvía continuas en el mapa",
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(
        "Siguiente paso: cuando valides esta línea, pásame la Línea 2 del mismo documento "
