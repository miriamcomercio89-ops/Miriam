# -*- coding: utf-8 -*-
"""
PDF completo — Línea D de tranvía (documento: «D»).
No es circular ni exprés: letra histórica del plano; recorrido diametral normal.
"""
...
OUT = Path(__file__).resolve().parent / "lineas" / "linea_D_tranvia.pdf"
...
LINE = {
    ...
    "code_doc": "D",
    "mode": "Tranvía",
    "name": "Plaza del Almendral – Estación María del Mar",
    "name_old": "Nußdorfer Platz → Hauptbahnhof",
    "color": "#9B2335",
    "text": "#FFFFFF",
    "kind": "Diametral / letra histórica (no circular)",
    "service": "05:45 – 23:45",
    "distance_km": 6.4,
    "runtime_min": 24,
    "headway_peak": 7,
    ...
    "vehicles": [
        ...
        ("ULF B1 (Viena)", "Base", 180, "refuerzo"),
    ],
    "depot": "Depot Süd / Depósito Sur",
    "zones": ["A", "B"],
    "notes": (
        "Línea D del documento: NO es especial por ser circular o exprés. "
        "La letra D es el código del plano (estilo Wiener Linien). En EMT la mantenemos "
        "como D. Une Almendral con la Estación por Cipreses, Ayuntamiento, Naranjos, "
        "Ópera, Artes, Teatro Cervantes y Labradores."
    ),
}

STOPS = [
    ("Nußdorfer Platz", "Plaza del Almendral", "Am Blumenmarkt", True, "B", True),
    ("Schwarzenbergplatz", "Plaza de los Cipreses", "Lilienstraße", False, "A", False),
...
def build():
    ...
    brows = [[Paragraph("<b>Franja</b>", STY["Tiny"]),
              ...
              Paragraph("<b>Ej. salidas cabecera Almendral</b>", STY["Tiny"])]]
    ...
    story.append(Paragraph(
        ...
        "Día de partido: útil hacia Estación para enlazar R-EST.",
        STY["Body"]))
    ...
    hubs_txt = [
        ("Plaza del Almendral", "Cabecera sur/oeste; buses de barrio."),
        ("Plaza del Ayuntamiento", "Casco; cruce L1/L2/L8/L10."),
        ("Plaza de la Ópera", "Hub L1/L8/L9/L10."),
        ("Estación María del Mar", "Cabecera este; S/U; L4/L5."),
    ]
    ...
    checks = [
        f"Crear línea código «{LINE['code']}» · HEX {LINE['color']}",
        "Renombrar las 9 paradas (varias ya existentes)",
        "Cabeceras: Plaza del Almendral ↔ Estación María del Mar",
        "Asignar Flexity + ULF B1",
        ...
        "Comprobar vía Almendral → casco → Estación en capa Tram",
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(
        "Siguiente del documento: O → Línea O.",
        STY["Small"]))
